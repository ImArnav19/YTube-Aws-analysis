#ETL Pipeline code where auto csv data fetched by data catalog then cleaned, schema transformation and 
# sent back to new s3 cleaned
...
predicate_pushdown = "region in ('ca','gb','us')" #used to just have utf-8 encoding

datasource0 = glueContext.create_dynamic_frame.from_catalog(database = "db_ytube", table_name = "raw_statistics", transformation_ctx = "datasource0", push_down_predicate = predicate_pushdown)
...
datasink4 = glueContext.write_dynamic_frame.from_options(frame = df_final_output, connection_type = "s3", connection_options = {"path": "s3://ytube-useast1-cleaned/youtube/raw_statistics/", "partitionKeys": ["region"]}, format = "parquet", transformation_ctx = "datasink4")
